import { getSessionUser } from "@/lib/auth/session";
import { SocialDashboard } from "@/components/social-dashboard";
import Link from "next/link";
import { LogoutButton } from "@/components/logout-button";

export const dynamic = "force-dynamic";

export default async function SocialPage() {
  const user = await getSessionUser();
  if (!user) {
    return (
      <main className="grid min-h-screen place-items-center bg-slate-950 px-6 py-12">
        <p className="text-slate-400">You must be logged in.</p>
      </main>
    );
  }

  return (
    <main className="min-h-screen bg-slate-950">
      <nav className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
        <div className="flex items-center gap-4">
          <Link href="/dashboard" className="flex items-center gap-1.5">
            <svg className="h-7 w-7 text-red-500" viewBox="0 0 24 24" fill="currentColor">
              <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814z" />
              <path d="M9.545 15.568V8.432L15.818 12l-6.273 3.568z" fill="white" />
            </svg>
            <span className="text-lg font-bold text-white">you2ube</span>
          </Link>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/dashboard" className="text-sm font-medium text-slate-400 hover:text-white">
            Dashboard
          </Link>
          <LogoutButton />
        </div>
      </nav>

      <div className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6">
          <p className="text-sm uppercase tracking-[0.08em] text-slate-500">Social</p>
          <h1 className="mt-2 text-3xl font-bold text-white">Friends & activity</h1>
          <p className="mt-2 text-sm text-slate-400">
            Send friend requests, accept or reject incoming requests, see who&apos;s online, and browse your activity feed.
          </p>
        </div>

        <SocialDashboard currentUserId={user.id} />
      </div>
    </main>
  );
}
